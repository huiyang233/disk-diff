...
### 1. 磁盘空间扫描 (Disk Space Scan)
> 极速多线程并发扫描，自底向上归集文件大小，直观以股市风格矩形树图可视化展示磁盘空间分布。

![磁盘空间扫描](docs/screenshots/image_scan.png)

### 2. 快照差异对比 (Snapshot Diff)
> 对比任意两份历史快照（或与当前内存扫描对比），通过子树智能剪枝与红绿热力图，精准揪出偷偷暴涨的目录与文件。

![快照差异对比](docs/screenshots/image_diff.png)
...
npm run tauri build
```
打包输出路径位于 `gui/src-tauri/target/release/bundle/`。
